...
function slideSection() {
  ...
  return (
    <Layout>
      ...
      <SideDrawer />
      <PercentageProgressIndicator percent={currentCaseStudySlideData.progressPercent ? currentCaseStudySlideData.progressPercent : '0%'} />
...
const QuestionResponseLayout = ({slideData, currentSlidePosition, navigationLeftHandler, navigationRightHandler}) => {
  ...
  return (
    ...
    style={{border: '1px solid black'}}>
      ...
      <Grid item xs={12} sm={1}  align="left" style={{border: '1px solid red'}}></Grid>
      ...
      <Grid item xs={12} sm={5}  align="left" style={{ border: '1px solid red', fontFamily: 'Poppins', fontSize: '1.5rem' }}>
...
const TaskLayout = ({slideData, currentSlidePosition, navigationLeftHandler, navigationRightHandler}) => {
  ...
  const centerButtonDivStyle = {
    ...
    const instructionTextStyle = { display: 'flex', flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'flex-start' }
    const additionalTextStyle = { display: 'flex', flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'flex-start', fontWeight: '400',fontSize:'0.75rem'}
    ...
    const bottomCenteredLayoutStyle = { display: 'flex', flexDirection: 'column',justifyContent: 'flex-end', alignItems: 'center',border: '1px solid red',height: '100px'}
  ...
  return (
    <section>
    {(showBackgroundVideo === true) ? <BackgroundVideoCustom ref={ref} onClick={togglePlayVideo} VideoHolder={VideoHolder} videoName={currentCaseStudySlideData.animationVideoName} playButtonState={state.videoPlaying}></BackgroundVideoCustom> : ''}
     
    <Grid container 
    spacing={0}
    spacing={0}
    justify="center"
    style={{position: 'relative',border: '1px solid black',height: '100vh' }}>
      <Grid item xs={12} sm={12}  style={{border: '0px solid red'}}>
          <div style={topSectionStyle}>
              ...
              {((currentCaseStudySlideData.slugName) === slideData.listenSection_ListenToDogHeart_Task_Dudley.slugName) ?  <div style={centerInDivStyle}><img src={soundOffIcon} alt="sound off" width="30" height="30"/></div> : ''}
          </div>
      </Grid>
      ...
      <Grid item xs={12} sm={3}  align="center" style={{border: '1px solid red',height: '75%'}}>
          <div style={{ display: 'flex', flexDirection: 'column',justifyContent: 'flex-end', alignItems: 'center',border: '1px solid red',height: '100%'}}>
              <div style={instructionTextStyle}>{(currentCaseStudySlideData.instructionsText ? processHtml(currentCaseStudySlideData.instructionsText)  : '')}</div>
              <div style={additionalTextStyle}><em>{(currentCaseStudySlideData.additionalText ? processHtml(currentCaseStudySlideData.additionalText)  : '')}</em></div>
          </div>
      </Grid>

      <Grid item xs={12} sm={1}  align="left" style={{border: '0px solid red'}}></Grid>

      <Grid item xs={12} sm={12}  style={{border: '1px solid red',height: '20%'}}>
    ...
    {((currentCaseStudySlideData.slugName) === slideData.listenSection_ListenToDogHeart_TaskInstructions_Dudley.slugName) ? <div style={centerButtonDivStyle}><DarkBlueRoundedButton buttonText={currentCaseStudySlideData.buttonLinks[0].title} onClickHandler={navigationRightHandler} /></div> : ''} 
    
    {(showVideoButton) ? <div onClick={togglePlayVideo} id="videoLargePlayBtn" style={videoPlayButtonStyle}><img src={videoPlayButtonIcon} ref={refPlayButton} alt="" style={{ width:'100px',height:'100px' }} /><img src={videoPauseButtonIcon} ref={refPauseButton} alt="" style={{ width:'100px',height:'100px',display:'none' }} /></div> : ''}
