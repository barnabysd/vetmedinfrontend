...
class UltrasoundContainer extends React.Component {
    ...
    render() {
      ...
      const moveStep = (param) => {
        ...
        this.state.stage = ultrasoundSteps.INFO_SPOTS_AORTA
        this.forceUpdate()
      }
      ...
      function drawLineAnimation1() {
        ...
        TweenLite.set(".path02",{opacity: 1})
        const action = new TimelineMax()
    ...
    const showStage3Tap2 = (event) => {
      ...
      const displayState1 = (stage) => { return (stage === ultrasoundSteps.MEASURE_INTERNAL_SHORT_AXIS_AORTA) ? 'block':'none' }
      ...
      if (typeof window !== 'undefined') {
         // if (this.state.stage === 1) hideIntro()
          if (this.state.stage === ultrasoundSteps.MEASURE_INTERNAL_SHORT_AXIS_AORTA) drawLineAnimation1()
      ...
      return  (
        ...
        function handleDragStart(e) {
          ...
          function handleCustomDrop(e) {
            ...
            const coords = "left" + e.currentTarget.style.left + " top "  + e.currentTarget.style.top
      ...
      ///if (this.state.stage === ultrasoundSteps.MEASURE_INTERNAL_SHORT_AXIS_AORTA) {
        //if (typeof window !== undefined) {
           /// window.addEventListener('DOMContentLoaded', () => {
            
                // var boxes = document.querySelectorAll('.smallInfoBoxes');
                // [].forEach.call(boxes, function(box) {
                ...
                //     box.addEventListener('drop', handleCustomDrop, false);
      ...
      return (<Layout>
        ...
        <Grid container  
            ...
            <Grid item xs={12} sm={12}  style={gridStyle}>
                ...
                {/* <div id="step0" style={{display: (this.state.stage === 1) ? 'flex':'none',flexDirection:'row',width:'100%',margin:'auto'}}>
                        <div id="introImage" style={{display:'flex',width:'50%',height:'100vh',flexDirection:'column',alignItems:'flex-end',justifyContent:'center'}}> 
                            <CustomFluidImage  style={{display: displayDog(this.state.dogName, dogName.DUDLEY), width:'500px',height:'500px'}} imgName="ultrasound-and-dudley-01.png" />
                            <CustomFluidImage  style={{display: displayDog(this.state.dogName, dogName.POPPY), width:'500px',height:'500px'}} imgName="ultrasound-and-dudley-01.png" />
                            <CustomFluidImage  style={{display: displayDog(this.state.dogName, dogName.REGGIE), width:'500px',height:'500px'}} imgName="ultrasound-and-dudley-01.png " />
                        </div>
                ...
                <Frame id="step1" style={{display: (this.state.stage > 0 && this.state.stage < 9 ) ? 'block':'none'}}>
                    <FrameInner>
                        ...
                        <div id="LinesHolder1" style={{display: displayStateStage(this.state.stage,ultrasoundSteps.MEASURE_INTERNAL_SHORT_AXIS_AORTA),position:'absolute',left:'19px', top:'-13px',width:'600px',height:'250px'}}>
                          <Lines2 style={{opacity:1,transform: 'rotate(90deg) translate(-112px, -179px)'}}/>
                        </div>

                        <div id="LinesHolder2" style={{display: displayStateLine01(this.state.stage),position:'absolute',left:'2%',top:'160px',width:'600px',height:'250px'}}>
                          <Lines />
                          </div>
                        ...
                        <BlueSmallInfoBox id="squarePv" className={"smallInfoBoxes"} style={{display:(this.state.hintChecked && this.state.stage === ultrasoundSteps.MEASURE_INTERNAL_SHORT_AXIS_AORTA) ? 'block' : 'none',left:"296px",top:"385px"}}>PV</BlueSmallInfoBox>
                        <BlueSmallInfoBox id="squareNc" className={"smallInfoBoxes"} style={{display:(this.state.hintChecked && this.state.stage === ultrasoundSteps.MEASURE_INTERNAL_SHORT_AXIS_AORTA) ? 'block' : 'none',left:'452px', top:'193px'}}>NC</BlueSmallInfoBox>
                        <BlueSmallInfoBox id="squareLa" className={"smallInfoBoxes"} style={{display:(this.state.hintChecked && this.state.stage === ultrasoundSteps.MEASURE_INTERNAL_SHORT_AXIS_AORTA) ? 'block' : 'none',left:'464px', top:'330px'}}>LA</BlueSmallInfoBox>
                        <BlueSmallInfoBox id="squareRc" className={"smallInfoBoxes"} style={{display:(this.state.hintChecked && this.state.stage === ultrasoundSteps.MEASURE_INTERNAL_SHORT_AXIS_AORTA) ? 'block' : 'none',left:'506px', top:'147px'}}>RC</BlueSmallInfoBox>
                        <BlueSmallInfoBox id="squareLc" className={"smallInfoBoxes"} style={{display:(this.state.hintChecked && this.state.stage === ultrasoundSteps.MEASURE_INTERNAL_SHORT_AXIS_AORTA) ? 'block' : 'none',left:'539px', top:'227px'}}>LC</BlueSmallInfoBox>
                        ...
                        <SlideText display={displayState7(this.state.stage)}
                              ...
                              failedText={"Try again"}
                              bodyText={""}
                              titleText={this.resources.field_bottomtitlestep6 ? this.resources.field_bottomtitlestep6.processed : ''} />
                        ...
                        <SwitchHolder id="switch" style={{display: displayStateSwitch(this.state.stage)}}>
                         ...
                         <TooltipHolder1 id="tip1" stageVisible={(this.state.stage === ultrasoundSteps.MEASURE_INTERNAL_SHORT_AXIS_AORTA)} hintChecked={this.state.hintChecked} textHtml={this.resources.field_taptooltiptext1.processed} leftPos = '22%' topPos = '29%' />
                         <TooltipHolder2 id="tip2a" stageVisible={(this.state.stage === ultrasoundSteps.MEASURE_INTERNAL_SHORT_AXIS_AORTA)} hintChecked={this.state.hintChecked} textHtml={this.resources.field_taptooltiptext2.processed} rightPos = '24%' topPos = '53%' />
                ...
                <PageSection id="step3" style={{display: (this.state.stage === 9) ? 'flex':'none'}}>
                        ...
                        <RightPageSection id="summaryText">
                            ...
                            <TaskSummaryFootnote>{processHtml(replaceDogName(this.resourcesSummary.field_tablefooterhtml1 ? this.resourcesSummary.field_tablefooterhtml1.processed : '',this.state.dogName))}</TaskSummaryFootnote>
                            <div>&nbsp;&nbsp;&nbsp;&nbsp;</div>

                            <VideoSmallWidget videoCaptionText={this.resourcesSummary.field_videocaptiontext1 ? this.resourcesSummary.field_videocaptiontext1.processed : ''} instance="One"/>
